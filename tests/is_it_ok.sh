...
#  2016-03-08  Pridana kontrola ulohy CLS a odebrana uloha CST (upozornil Michal Ormo�)
#  2017-02-06  Zrusen pozadavek na metainformace na tretim radku skriptu, zakomentovan kontrolni kod
#  2017-03-04  Aktualizace prikazu pro PHP 5.6 a Python 3.6 na Merlinovi (upozornil �ubo� Hlipala)
...
if [[ -n $3 ]]; then
  if [[ $3  = "--force" ]]; then
    ...
    if [[ $TASK -eq 1 ]]; then
      ...
      REQUIRED_SCRIPTS=( $INTERPRETSCRIPT )
...
function unpack_archive () {
  ...
  if [[ $RETCODE -eq 0 ]]; then
    ...
    echo_color red "ERROR (code $RETCODE)"read
    exit 1
  fi
} 
...
if [[ -f rozsireni ]]; then
  ...
  RETCODE=$?
  if [[ $RETCODE = "0" ]]; then
    UNKNOWN=`cat rozsireni | grep -v -E -e "^(STATP|NVP|FLOAT|STACK|STATI)$" | wc -l`
    if [[ $UNKNOWN = "0" ]]; then
      echo_color green "OK" 
    else
